          ...
          <div class="col-xs-12">
            <div class="box box-info">
              <div class="box-header">
                <h3 class="box-title">Short Listed Applicant <span class="countnumber"><small class="textcount">{{count($countsortApplicant)}}</small></span> </h3><div class="rightportion"><a  data-toggle="modal" data-target="#emailmodal" data-type="bulk" class="common-button pull-right commonmailsort" style="display:none">Sendmail</a><a  data-toggle="modal" data-target="#emailmodal" data-type="bulk" class="common-button pull-right commonmail">Sendmail</a><a href="{{route('careerjob.detail',['id'=>$jobid])}}" class="common-button pull-right">Job Details</a><a href="{{route('career.applicantlist',['id'=>$jobid])}}" class="pull-right common-button">Go Back</a>@if(count($findShortlistedapplicant) > 0)<a href="{{route('zip.shortlisted',['id'=>$jobid])}}" class="pull-right common-button cmnfilter">Export PDF</a>@endif<button  class="pull-right filteritemPdf common-button" style="display:none">Export PDF</button><button class="exel pull-right common-button" style="display:none">Excel</button></div>
                  ...
                  <div class="col-md-12" style="margin-bottom:10px;">
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        ...
                        <input type="hidden" class="form-control" id="jobbid" name="id" value="{{$jobid}}">
              ...
              <div class="box-body hiden">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                     <th width="15%">Name</th>
                     <th width="15%">Emailil</th>
                     <th width="15%">Mobile</th>
                     <th width="15%">Action</th>
                     <th width="20%">Send</th>
                  </tr>
                  </thead>
                  <tbody class="jobappend" id="applicantlistforemail">
                    @foreach($findShortlistedapplicant as $sorted)
                   <tr>
                   <td><a href="{{route('application.show',['id'=>$sorted->cvformsid])}}">{{$sorted->name}}</a></td>
                      <td class="bulkemail" id="{{$sorted->email}}">{{$sorted->email}}</td>
                      <td>{{$sorted->mobile}}</td>
                      <td>
                        <a class="unsort" data-id="{{$sorted->shortlistid}}"><span class="glyphicon glyphicon-trash btn btn-danger btn-sm"></span></a>
                      </td>
                     <td><a  data-toggle="modal" data-target="#emailmodal" data-id="{{$sorted->email}}" data-type="single" class="common-button singlecommonmail">Sendmail</a></td>
                    </tr>
                  @endforeach
                  </tbody>
                </table>
...
$(document).on('submit','#mailapplicant',function(e){
    ...
    $(document).on('submit','#searchform',function(e){
      ...
      if(area =='' && age =='' &&  cgpa == '' && gender== '' && degree== '' && degree==''){
        ...
        $.ajax({
              url:"{!! route('shortapplicant.filter') !!}",
              method:"POST",
                  ...
                  success:function(data)
                  {
                    ...
                    $.each(data,function(index,filterdata){ 
                      ...
                      $('.filterdataapp').append(`<tr id="`+filterdata.userid+`">
                        ...
                        <td>
                          <a class="filtunsort" data-jobid="`+jobid+`" data-userid="`+filterdata.userid+`"><span class="glyphicon glyphicon-trash btn btn-danger btn-sm"></span></a>
                        </td>
...
$(document).on('click','.unsort',function(e) {
    ...
    if (result.value) {
      ...
      $(document).on('click','.filtunsort',function(e) {
          e.preventDefault();
          ...
          if (result.value) {
              $.ajax({
              url: "{!! route('filtershortlisted.remove') !!}",
              type: "get", 
              data: {  
                jobid: jobid, 
                userid:userid,
     ...
     $(document).on('click','.filteritemPdf',function(e){
      ...
      data: {  
          ...
          success: function(response) {
            //console.log('success');
            window.location='/sortfiltered.zip';
          }
        });
      //console.log(ids);
